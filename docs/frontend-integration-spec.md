# DvP Exchange — Frontend Integration Spec

**Audience:** any client integrating with the exchange. **Backend:** an Egypt-L1 cluster; point the
client at your own cluster's validator endpoints (configure them in your deploy manifest — no node
addresses are hard-coded in this document). Every method, argument/return shape, seeded id, and the
exact call sequence per flow is documented below, so a client builds against it with **zero backend
guesswork**.

> Author: Thebes core team. Source-of-truth `.did` files: `smart-contracts/dvp-matching/build/Matching.did`,
...
**Binding seeded assets to passkey principals (one-time setup step).** The seeded balances
...
3. **Tamper-evidence:** `coreD.auditRootHex()` → the on-chain MMR root; re-derive it off-chain from the `encoded` events (leaf = `SHA256(0x00‖utf8(encoded))`, node = `SHA256(0x01‖L‖R)`, fold peaks high→low) and assert equality. Reference re-deriver: `examples/mmr_rederive.py`.
...
- Clearing math: uniform clearing price p* per window (`ClearResult.clearingPrice`); the long side is rationed by **price-time priority** (the implemented allocation; a pro-rata variant is a possible future option).
